...
#     Version     : 1.0.0
#     Description : Check TLS version supported on endpoints
#     Contributors:
######################################################################################
...
if [ $# -eq 1 ] && [ -f "$1" ]; then
    ...
    echo "Usage:"
    echo "  $0 filename                     # To check TLS versions for domains in a file"
    echo "  $0 domain:port                  # To check TLS versions for a single domain"
    echo "  $0 domain1:port  domain2:port   # To check TLS versions for multiple domains"
    exit 1
fi
